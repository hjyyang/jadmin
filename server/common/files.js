...
class Files {
						...
						files.forEach((item) => {
							...
							if (stat.isDirectory() === true) {
								...
								if (fileName.indexOf(".") != 0) {
									//排除.开头的文件
									if (type.length > 0) {
										if (type.includes(fileName.split(".")[1])) {
											fileObj["name"] = fileName;
											fileUrl = fPath.split("static")[fPath.split("static").length - 1];
											var re = new RegExp("\\\\", "g");
	...
	delete(path) {
		return new Promise(function (resolve, reject) {
			fs.unlink(node_path.resolve(__dirname, "../../static" + path), function (err) {
				if (err) {
					resolve(false);
				}
				resolve(true);
	...
	isExists(path) {
		let stat = fs.existsSync(node_path.resolve(__dirname, "../../static" + path));
		if (stat) {
			//已存在改路径文件
	...
	rename(oldPath, newPath) {
		let self = this;
		return new Promise(function (resolve, reject) {
			let exist = fs.existsSync(node_path.resolve(__dirname, "../../static" + oldPath));
			if (!exist) {
				//原地址不存在
				resolve(false);
			}
			let result = self.isExists(newPath);
			fs.rename(node_path.resolve(__dirname, "../../static" + oldPath), node_path.resolve(__dirname, "../../static" + result), (err) => {
				if (err) {
					resolve(false);
				}
				resolve(result);
...
var storage = multer.diskStorage({
	...
	destination: function (req, file, cb) {
		let siteDate = new Date(),
			...
			dir2,
			targetDir;
		dir = node_path.resolve(__dirname, uploadPath + "/" + siteDate.getFullYear());

		if (siteDate.getMonth() > 9) {
			dir2 = dir + "/" + (siteDate.getMonth() + 1);
			targetDir = "/" + siteDate.getFullYear() + "/" + (siteDate.getMonth() + 1);
		} else {
			dir2 = dir + "/0" + (siteDate.getMonth() + 1);
			targetDir = "/" + siteDate.getFullYear() + "/0" + (siteDate.getMonth() + 1);
		}
